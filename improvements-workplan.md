...
- **MUX bleed — samples attributed to the wrong channel** — confirmed on real hardware 2026-07-20: raw EMG columns intermittently read ~0 (the true value of the envelope channel converted immediately before them) while envelope columns intermittently read raw-magnitude values (~600), even though `#CNT` showed correct, symmetric per-channel counts (20:1 fast:slow ratio, matching across all 4 ADCs) — proving the *scheduling* was correct and the *bug was in which channel a given conversion actually reflects*. Per the ADS1015 datasheet (§9.3.3), the first conversion completed after a MUX switch still reflects the *previous* channel; because this round-robin's fast channels always alternate (`ch0, ch2, ch0, ch2, ...`) and slow channels are injected as one-off substitutions, **the MUX switches on effectively every service call**, so effectively every sample — not "a fraction" as originally scoped here — was one channel behind its label. Fixed: a `settling_` flag set right after every `writeConfig()` mux switch (including the very first conversion after `start*()`, since the chip's prior state doesn't match either); the next DRDY-triggered conversion is read (to clear the ALERT condition) but discarded — not attributed, not counted, not delivered to the callback. Applied to all three service paths (`serviceConversion()`, `mixedContinuousTask()`, `continuousTask()`) for consistency, though only `serviceConversion()` is on the bracelet's live path. `getEffectiveSampleRate()` updated to reflect the resulting ~2× throughput cost of any multi-channel round-robin (single-channel configs are unaffected — the MUX never moves, so nothing is discarded).
  **Superseded — the discard was not sufficient.** Hardware retest still showed raw/envelope values swapping columns at random (though `#CNT` stayed perfectly symmetric, again confirming scheduling was fine). Root cause is deeper than "one stale sample": per TI, in continuous mode the MUX change takes effect only *after* the conversion already in progress completes, so whether a config write yields one stale conversion or zero depends on where it lands relative to that internal boundary — a genuine race against I2C timing jitter. A fixed discard-one rule wins that race about half the time, and each miss shifts attribution by one channel until another miss shifts it back, which is exactly the drifting scramble observed. See "Single-shot round-robin" below for the actual fix.
- **Channel attribution race → single-shot round-robin** — replaced free-running continuous mode with single-shot triggered sampling on the mixed-rate external path (the one the bracelet uses). Each conversion is explicitly requested for a named channel (`OS=1`, `MODE=single-shot`, MUX=ch), so its result can only belong to that channel — attribution is structural rather than inferred from timing, and no pipeline exists to desync. TI's own guidance is that single-shot is the recommended mode when input channels are swapped frequently; the same symptom is reported by other projects using continuous-mode round-robin (e.g. ESPEasy issue #3159, "results of multiple configured channels get sporadically mixed"). Supporting safeguards added at the same time: I2C errors on the conversion read no longer masquerade as a real `0` reading (the sample is dropped and counted instead); a failed trigger write leaves nothing in flight rather than desyncing the shadow state; `retriggerIfStalled()` re-arms a channel whose trigger or DRDY edge was lost (naming the channel explicitly, so recovery can't corrupt labels), driven by a 10 ms timeout in each bus service task; and `#CNT` now also reports per-ADC I2C-error and retrigger counts so bus trouble is visible instead of silent. Throughput improves as a side effect — single-shot wastes no conversion, so All mode goes to ~1100 Hz/ch raw (vs ~825 with continuous+discard).
  **Verify on hardware:** raw columns should sit consistently near the true level (~600 at rest) and envelope columns near 0, with no swapping; `#CNT` should keep its 20:1 fast:slow ratio and cross-ADC symmetry, and report 0 (or near-0) I2C errors and retriggers.
